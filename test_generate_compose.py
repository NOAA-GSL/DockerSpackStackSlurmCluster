...
class GenerateComposeTests(unittest.TestCase):
    def test_default_generates_five_nodes(self):
        result, content = run_generator()
        self.assertEqual(result.returncode, 0, result.stderr)
        for n in (1, 2, 3, 4, 5):
            self.assertIn(f"slurmnode{n}:", content)
            self.assertIn(f"container_name: ${{CLUSTER_NAME:-spack-stack}}-node{n}", content)
    ...
    def test_custom_node_count(self):
        result, content = run_generator("--nodes", "3")
        self.assertEqual(result.returncode, 0, result.stderr)
        for n in range(1, 4):
            self.assertIn(f"slurmnode{n}:", content)
    ...
    def test_only_first_node_has_build_section(self):
        result, content = run_generator("--nodes", "3")
        self.assertEqual(result.returncode, 0, result.stderr)
        node1 = content.split("slurmnode1:", 1)[1].split("slurmnode2:", 1)[0]
        node2 = content.split("slurmnode2:", 1)[1].split("slurmnode3:", 1)[0]
        self.assertIn("build:", node1)
        self.assertNotIn("build:", node2)

    def test_frontend_and_master_use_cluster_name(self):
        result, content = run_generator()
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("slurmfrontend:", content)
        self.assertIn("container_name: ${CLUSTER_NAME:-spack-stack}-frontend", content)
        self.assertIn("slurmmaster:", content)
        self.assertIn("container_name: ${CLUSTER_NAME:-spack-stack}-master", content)
