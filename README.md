...
By default the cluster containers are named `spack-stack-frontend`,
`spack-stack-master`, `spack-stack-node1`, etc., and no host ports are
published, so a second cluster can be started alongside the first as long
as it uses a different Compose project name and `CLUSTER_NAME`:
```
CLUSTER_NAME=cluster2 docker compose -p cluster2 -f docker-compose.yml up -d --pull never
```
The `-p` flag keeps the network and volumes isolated from other instances,
and `CLUSTER_NAME` keeps the container names unique (`cluster2-frontend`,
`cluster2-master`, ...), so you can still run:
```
docker exec -it cluster2-frontend bash -l
```
